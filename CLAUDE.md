...
- 本项目为个人站点，日常小改动直接在 `main` 分支上提交，不必为每个小功能新建独立分支。
- 只有涉及较大重构、实验性功能或需要长时间隔离的改动，才考虑新建分支。
- 提交信息使用中文，遵循约定式提交（Conventional Commits），末尾保留 `Co-Authored-By: Claude <noreply@anthropic.com>`。
