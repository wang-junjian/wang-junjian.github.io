...
基于项目文档和代码结构分析，Kilo是一个开源的AI编码代理平台，主要用于加速软件开发过程。它是OpenCode项目的fork版本，增强为全面的agentic工程平台。以下是其核心能力的详细分析：

### 1. **AI驱动的代码生成与自动化**
   - **核心功能**：支持从自然语言描述生成代码，例如用户输入"add input validation to the signup form"，代理会自动生成相应的代码片段。
   - **自动化任务**：能够自动化重复性编码任务，如重构代码、运行测试、修复错误等。代理会自我检查工作，确保代码质量。
   - **内联自动完成**：提供实时AI驱动的代码补全建议，提升编码效率。
   - **示例**：在CLI中使用`kilo run "add input validation to the signup form"`命令，代理会解析任务、生成代码并应用更改。
...
### 3. **终端与浏览器自动化**
   - **终端命令执行**：代理可在用户授权下运行shell命令，如构建、测试、部署等。
...
### 4. **多模式操作**
   ...
   - **模式切换**：代理可根据任务自动或手动切换模式，增强适应性。

### 5. **扩展能力与协议**
   - **MCP服务器市场**：通过Model Context Protocol（MCP）扩展代理能力，用户可轻松发现和使用MCP服务器。
...
- **生命周期**：实例在请求时初始化，包含配置、代理、工具和服务。
- **清理机制**：支持SIGTERM/SIGINT信号处理，确保资源释放（`Instance.disposeAll()`）。

#### **代理执行引擎（Agent Execution Engine）**
- **代理定义**：`packages/opencode/src/agent/agent.ts` 定义代理信息（Info），包括模式（primary/subagent/all）、权限规则（Permission.Ruleset）和模型配置。
- **内置代理**：
  - `code`（原build）：默认编码代理，支持工具执行。
  - `plan`：规划模式，禁用编辑工具。
  - `ask`：问答模式（Kilo扩展）。
  - `debug`：调试模式（Kilo扩展）。
  - `architect`：架构规划（Kilo扩展）。
- **代理选择**：基于配置或默认代理，权限合并用户和默认规则。
- **模式切换**：代理可根据任务自动或手动切换模式，限制工具访问（例如Architect模式仅允许编辑.md文件）。
...
- **模型选择**：基于代理配置选择模型，支持流式生成（`streamObject`）。
- **认证**：支持API密钥和OAuth，密钥存储在本地。

#### **通信协议**
- **REST API**：处理同步请求，如获取代理列表、执行命令、读取文件。
...
3. **代理调度**：根据请求选择代理，检查权限。
4. **工具执行**：代理调用工具，执行异步操作，实时SSE推送状态。
