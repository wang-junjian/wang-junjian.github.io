...
- [X] [不再迷信大一统模型！OpenAI平台工程负责人自曝：单模AGI路线已破灭！开源杀不死API！提示词工程依旧重要，揭秘如何定价](https://mp.weixin.qq.com/s/fWrFOivN78shcxyi1nnhlw)
  - 大一统模型不是终局，未来是多种专业模型并存。
  - OpenAI 正从“做最强模型”转向“做最强平台”。
  - 模型具有不可替换的粘性，难以被软件抽象层代替。
  - 强化微调（RFT）让企业能用自有数据把模型调到行业顶尖。
  - Prompt 已升级为更复杂的“上下文工程”，包含工具与检索。
  - 节点式 Agent 为强确定性和流程化场景提供必要的约束。
  - 按使用量计费将长期主导，短期内难以被“结果定价”取代。
  - 开源权重不会蚕食 API，因为推理与工程能力难以复制。
  - 多模态（如 Sora）与世界模拟能力将成为新的核心竞争力。
- [X] [地平线苏箐给智驾“泼冷水”：又进入苦日子阶段了（附演讲全文）](https://mp.weixin.qq.com/s/MSZpl_BCaJjl02Vn-k8B0w)
