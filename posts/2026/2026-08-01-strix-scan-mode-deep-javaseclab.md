...
**漏洞详情页的概念说明，结合示例（`vuln-0001.md`）**

| 二级标题 | 核心作用 | 面向读者 | 简要说明（结合你这份SSRF漏洞） |
| --- | --- | --- | --- |
| Description<br>漏洞概述 | 一句话清晰**定义漏洞是什么、触发入口、核心风险**；给非技术管理者快速看懂。 | 研发负责人、安全评审、项目经理 | 介绍 `/ssrf/vul` 接口存在未校验URL，可触发SSRF，概括风险场景，不深入代码。 |
| Evidence<br>证据 | **证明漏洞真实存在**，提供原始依据（代码片段、请求证据），支撑漏洞结论，避免“理论漏洞”争议。 | 开发、安全审核、复测人员 | 贴出危险代码行：直接使用用户输入构造`URLConnection`，是漏洞根因原始证据。 |
| Impact<br>危害/影响 | 描述**漏洞被成功利用后可造成的实际业务/资产损失**，支撑风险等级（CVSS/高危）判定。 | 管理层、风险评审、业务负责人 | 说明攻击者可窃取云元数据凭证、内网扫描、读取本地敏感文件，解释为什么标记为高危。 |
| Technical Analysis<br>技术分析 | 解释**漏洞底层原理、为什么防护失效**，串联代码逻辑与攻击面。 | 开发、安全工程师 | 分析`URLConnection`缺少协议校验、IP黑名单、重定向限制，解释为何能访问内网/169.254元数据。 |
| Proof of Concept（PoC）<br>概念验证 | 提供**可直接复现的步骤、Payload、脚本**；任何人拿到后都能自行验证漏洞。 | 开发、测试、安全复测人员 | 多条测试URL + Python脚本，覆盖云元数据、本地探测、文件读取、盲打；用于复测确认漏洞。 |
| Code Analysis<br>代码分析 | **精准定位漏洞文件、方法、行号**，给出对比式修复代码，方便开发直接修改。 | 开发工程师 | 定位`SsrfController.java`漏洞函数，给出不安全代码与修复代码对比，提供白名单、协议校验等改造方案。 |
| Remediation<br>修复/缓解方案 | 给出**可落地、完整的整改措施**（代码、网络、配置、安全策略）。 | 开发、运维、安全负责人 | 白名单域名、阻断内网IP、限制协议、关闭自动重定向、超时控制；长期防护策略。 |
| Assumptions<br>前提假设 | 写明**漏洞利用必须满足的环境条件**；说明什么情况下风险会降低甚至不可利用，避免高估/低估风险。 | 风险评审、运维、架构师 | 假设接口公网可达、服务器能访问内网/云元数据、无出站ACL/WAF；若条件不满足，攻击难度上升。 |

- 补充区分几组容易混淆的概念
    1. **Evidence vs PoC**
        - Evidence：原始证据（代码片段、抓包、日志），用于**证明漏洞客观存在**；
        - PoC：完整复现流程/脚本，用于**主动重现漏洞**，偏向操作与测试。
    2. **Technical Analysis vs Code Analysis**
        - Technical Analysis：偏原理、攻击面、安全机制缺失；
        - Code Analysis：偏精准代码定位、逐行修复方案，更贴近开发改造。
    3. **Impact vs Remediation**
        - Impact：讲“出事之后有什么损失”；
        - Remediation：讲“怎么做防止出事”。
